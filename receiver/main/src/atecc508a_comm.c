...
esp_err_t atecc508a_send(ATCAPACKE_t *packet){
    ...
    atecc508a_crc_update(&ctx, (uint8_t *)&packet + 1, sizeof(packet) - 1);
    uint16_t crc = 0;
    atecc508a_crc_end(&ctx, &crc);
    // wake up the device
    atecc508a_wake_up();
    ESP_LOGI(LOG_TAG, "> Sending command");
    // send the data
    ...
    // header
    i2c_master_write(cmd, (uint8_t *)&packet, sizeof(packet), 1);
    // crc
...
{
    ESP_LOGI(LOG_TAG, "Sending command 0x%02X param1=0x%02X, param2=0x%04X", command, param1, param2);
    ...
    uint8_t total_transmission_length = 1 + 1 + 1 + sizeof(param1) + sizeof(param2) + length + sizeof(uint16_t);
        ...
        uint8_t header[] = {
            ...
            param1,//param1
            (param2 & 0xFF),//param2
            (param2 >> 8) & 0xFF,//data[130]
            HMAC_RSP_SIZE};//rxsize
            

/*
        uint8_t header[] = {
            0x03,//word_address
            total_transmission_length - 1,//txsize
            command,//opcode
            param1,//param1
            (param2 & 0xFF),//param2
            (param2 >> 8) & 0xFF};//data[130]
...
esp_err_t atecc508a_receive(uint8_t *buffer, size_t length)
{
    ...
    ESP_LOGI(LOG_TAG, "< Response received (%d bytes, %d retries)", received, retries);
...
esp_err_t atecc508a_read(uint8_t zone, uint16_t address, uint8_t *buffer, uint8_t length)
{
    ...
    atecc508a_send_command(ATECC508A_CMD_READ, zone, address, NULL, 0);
...
esp_err_t atecc508a_write(uint8_t zone, uint16_t address, uint8_t *buffer, uint8_t length)
{
    ...
    atecc508a_send_command(ATECC508A_CMD_WRITE, zone, address, buffer, length);
